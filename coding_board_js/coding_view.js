...
var num = getParameterByName('no');
  ...
  var contentsList = JSON.parse(localStorage.getItem("comments2"));
  ...
  function warnEmpty() {
    ...
    function submitComment() {
     ...
     var commentjson= localStorage.getItem('comments2');
     var comment;
     if (!commentjson) {
       comment={};
     } else {
     ...
     localStorage.setItem('comments2', JSON.stringify(comment));
     drawRows(list);
  }
    ...
    function drawRows(list) {
      var templates = '';
      var body = document.getElementById('comments2-rows');
   ...
   function leftPad(value) {
    ...
    function removeComments(i) {
     ...
     var commentjson= localStorage.getItem('comments2'); // 로컬스토리지에서 모든 코멘트를 가져온다.
     ...
     localStorage.setItem('comments2', JSON.stringify(comment));  //바인딩된 코멘트를 LOCALSTORAGE에 저장
